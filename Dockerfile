...
RUN \
  ...
  if [ -z ${APP_VERSION+x} ]; then \
    APP_VERSION=$(curl -s "https://api.github.com/repos/Whisparr/Whisparr/releases/latest" \
    | awk '/tag_name/{print $4;exit}' FS='[""]'); \
  fi && \
  curl -Lo \
    /tmp/whisparr.tar.gz \
    "https://github.com/Whisparr/Whisparr/releases/download/${APP_VERSION}/Whisparr.${APP_VERSION#v}.linux-musl-x64.tar.gz" && \
  tar xzf \
